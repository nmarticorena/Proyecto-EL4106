...
def cga(fobj,obj, bounds,angle_in, mut=0.1, crossp=0.6,nk=20, popsize=1000, its=20,pcj=0.5,pmj=1):
...
class Generator(object):
    ...
    def Iterator(self,its,nk,lista):
        ...
        self.Reset_Arm()
        self.Show(lista[i0],nk,i0)
        rospy.sleep(10)
        self.Reset_Arm()
        self.Show(lista[i1],nk,i1)
        rospy.sleep(10)
        self.Reset_Arm()
        self.Show(lista[i2],nk,i2)
        rospy.sleep(10)
        self.Reset_Arm()
        self.Show(lista[i3],nk,i3)
        rospy.sleep(10)
        self.Reset_Arm()
    def Reset_Arm(self):
    ...
    def Show(self,lista,nk,indice):
        ...
        for i in range(nk):
            self.command1.data=lista[:-1][0][i][0]
            self.command2.data=lista[:-1][0][i][1]
            self.command3.data=lista[:-1][0][i][2]
            self.command4.data=lista[:-1][0][i][3]
            self.pub1.publish(self.command1)
            self.pub2.publish(self.command2)
            self.pub3.publish(self.command3)
            self.pub4.publish(self.command4)
...
if __name__ == '__main__':
    ...
    its=500
    angle_in=[0,0,0,0]
    l=list(cga(fitC,ob,bounds,angle_in,nk=nk,its=its))
    Generator(lista=l,nk=nk,its=its)
